...
const moduleData = ref<DatasetHealthsheetComposition>({
  composition: [],
});
...
onBeforeMount(async () => {
  ...
  const response = await fetch(
    `${baseURL}/study/${studyId}/dataset/${datasetId}/healthsheet/composition`,
    {
      method: "GET",
    }
  );
  ...
  const records = JSON.parse(data.composition);

  if (records.length > 0) {
    moduleData.value.composition = records;
  } else {
    moduleData.value.composition = QUESTIONS_JSON;
  }
});
    ...
    if (!errors) {
      ...
      const data = {
        composition: JSON.stringify(
          moduleData.value.composition.map((record) => ({
            id: record.id,
...
<template>
  <main class="flex h-full w-full flex-col pr-6">
    ...
    <FadeTransition>
      ...
      >
        <n-form-item
          v-for="record in moduleData.composition"
          :key="record.id"
